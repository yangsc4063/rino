 ...
 *
 * This graph assumes that vertices are numbered. In addition, the vertices numbers have to be consecutive starting from 0.
 *
...
public:
    ...
    void addVertex(const int &id) {
        if (id < adj_list_.size()) {
            std::cout << "\033[1;31mVertex already exists.\033[0m" << std::endl;
        } 
        else {
    ...
    void addEdge(const int &vertex_1, const int &vertex_2) {
        if (hasEdge(vertex_1, vertex_2)) {
            std::cout << "\033[1;31mEdge already exists.\033[0m" << std::endl;
            return;
        }
    ...
    void removeEdge(const int &vertex_1, const int &vertex_2) {
        if (vertex_1 >= adj_list_.size() || vertex_2 >= adj_list_.size()) {
            std::cout << "\033[1;31mTrying to remove non-existent edge.\033[0m" << std::endl;
            return;
        }
